...
import { ArrowRight, BookOpen, BrainCircuit, CheckCircle2, RadioTower, Shield, Sparkles, Trophy } from "lucide-react";
            ...
            >
              ...
              <div>
                ...
                <div className="mt-8 space-y-4">
                  {slides[currentSlide].bulletPoints.map((point, index) => (
                    <div key={index} className="flex items-center gap-3 text-lg font-semibold text-slate-700">
                      <CheckCircle2 className="text-brandsecondary shrink-0" size={24} />
                      <span>{point}</span>
                    </div>
                  ))}
                </div>
                ...
                <div className="mt-10 flex flex-col gap-4 sm:flex-row">
                  <Link 
                    className="inline-flex items-center justify-center gap-2 rounded-lg bg-brandprimary px-8 py-4 text-base font-bold text-white shadow-soft transition hover:-translate-y-0.5 hover:bg-indigo-600 active:scale-95" 
                    to={slides[currentSlide].primaryUrl}
                  >
                    {slides[currentSlide].primaryBtn}
                  </Link>
                  <Link 
                    className="inline-flex items-center justify-center gap-2 rounded-lg border border-slate-300 bg-white px-8 py-4 text-base font-bold text-slate-700 transition hover:-translate-y-0.5 hover:border-brandprimary/60 hover:bg-indigo-50 active:scale-95" 
                ...
                <div className="mt-6 flex gap-2">
                  {slides.map((_, index) => (
                    <button
                      ...
                      className={`h-3 rounded-full transition-all duration-300 ${index === currentSlide ? "w-8 bg-brandprimary" : "w-3 bg-slate-200 hover:bg-slate-300"}`}
                    ...
                    <div className="bg-white rounded-2xl overflow-hidden border border-slate-100 shadow-soft flex flex-col h-full group transition-all hover:shadow-md">
                      ...
                      <div className="relative h-48 overflow-hidden bg-slate-100">
                        ...
                        <div className="absolute bottom-4 left-4 bg-white/95 backdrop-blur-sm rounded-lg p-2 flex items-center gap-2 shadow-sm border border-slate-100/50 max-w-[calc(100%-2rem)]">
                          <div className="w-8 h-8 rounded-full bg-brandprimary/10 flex items-center justify-center font-black text-xs text-brandprimary shrink-0">
                            {program.logo}
                          </div>
                      ...
                      <div className="p-6 flex flex-col flex-1">
                        ...
                        <Link
                          to="/courses"
                          className="w-full text-center py-3 rounded-lg border border-slate-200 text-sm font-bold text-brandprimary hover:bg-indigo-50 hover:border-brandprimary/30 transition-all active:scale-95 mt-auto"
                        >
                          View Program
                        </Link>
                      </div>
